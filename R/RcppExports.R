...
# @references Nathan, R., Klein, E., Robledo-Arnuncio, J. J. and Revilla, E.
# (2012).  \code{Dispersal kernels: review} in Dispersal Ecology and Evolution,
...
kern_gaussian <- function(dist, scal) {
    .Call("recruitR_kern_gaussian", PACKAGE = "recruitR", dist, scal)
}

#' @describeIn kernels Exponential kernel.
#' @export
kern_exponential <- function(dist, scal) {
    .Call("recruitR_kern_exponential", PACKAGE = "recruitR", dist, scal)
}

#' @describeIn kernels Dispersal kernels from the exponential-power family.
#' @export
kern_exponential_power <- function(dist, scal, shap) {
    .Call("recruitR_kern_exponential_power", PACKAGE = "recruitR", dist, scal, shap)
}

#' @describeIn kernels 2Dt kernel.
#' @export
kern_2Dt <- function(dist, scal, shap) {
    .Call("recruitR_kern_2Dt", PACKAGE = "recruitR", dist, scal, shap)
}
