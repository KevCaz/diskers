...
res3 <- integrate(function(x) x*kern_exponentiel_power(x,2,3), 0, Inf)$value*p2
res4 <- integrate(function(x) x*kern_2Dt(x,2,3), 0, Inf)$value*p2
res5 <- integrate(function(x) x*kern_lognormal(x,2,3), 0, Inf)$value*p2
...
test_that("checking errors", {
  ...
  expect_error(kern_exponentiel_power(0, 0, 2))
